    ...
    return GestureDetector(
      ...
      onHorizontalDragEnd: (_) async {
        ...
        if (widget.controller.position > 0.5) {
          widget.controller.animationController.forward(from: widget.controller.position).then((value) async {
            widget.onAnimationEnd();
            widget.controller.animationController.reverse(from: widget.controller.position);
          });
        } else {
          widget.controller.animationController.reverse(from: widget.controller.position);
        }
      ...
      child: Obx(
        () => AnimatedContainer(
          ...
          decoration: BoxDecoration(
              color: widget.controller.status == SwipeButtonStatus.normal || widget.controller.status == SwipeButtonStatus.loading
                  ? ColorManger.blueColor
                  : widget.controller.status == SwipeButtonStatus.success
                      ? Colors.green
                      : Colors.red,
              borderRadius: BorderRadius.circular(AppSize.s30)),
          width: widget.controller.status == SwipeButtonStatus.normal ? 351.w : 40.w,
          height: widget.controller.status == SwipeButtonStatus.normal ? 54.h : 40.h,
          child: AnimatedBuilder(
            animation: widget.controller.animationController,
            builder: (context, child) => Obx(
              () => Transform.translate(
                offset: Offset(
                  (widget.controller.isAnimating ? widget.controller.position : widget.controller.animation.value) * 100,
                  0.0,
                ),
                child: child,
              ),
            ...
            child: AnimatedCrossFade(
              duration: const Duration(milliseconds: 250),
              crossFadeState:
                  widget.controller.status == SwipeButtonStatus.loading ? CrossFadeState.showFirst : CrossFadeState.showSecond,
              firstChild: Center(
                child: SizedBox(
                  width: 20.w,
                  height: 20.h,
                  child: const CircularProgressIndicator(color: ColorManger.white),
                ),
              ),
              ...
              secondChild: Center(
                child: AnimatedCrossFade(
                  duration: const Duration(milliseconds: 250),
                  crossFadeState: widget.controller.status == SwipeButtonStatus.success
                      ? CrossFadeState.showFirst
                      : CrossFadeState.showSecond,
                  firstChild: Icon(Icons.check, color: ColorManger.white, size: 35.w),
                  reverseDuration: const Duration(milliseconds: 250),
                  firstCurve: Curves.linear,
                  secondCurve: Curves.linear,
                  secondChild: widget.controller.status == SwipeButtonStatus.failure
                      ? Icon(Icons.close, color: ColorManger.white, size: 35.w)
                      : Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            SvgPicture.asset(IconsAssets.startTripArrow, width: 29.w, height: 13.h),
                            8.horizontalSpace,
                            CustomText(
                                text: el.tr(AppStrings.swipeToStartTheTrip),
                                style: Theme.of(context).textTheme.bodyLarge!.copyWith(color: ColorManger.white)),
                          ],
                        ),
                ),
              ),
